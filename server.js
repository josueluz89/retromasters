...
const manifest = {
  ...
  description: 'Canales de Costa Rica, Colombia, España, Pluto TV LATAM y ViX Gratis en vivo',
...
app.get('/health', (req, res) => res.json({ status: 'ok', channels: 'CR+CO+ES+PL+VIX', time: new Date().toISOString() }));

channels.init().then(() => {
  app.listen(PORT, () => {
    console.log(`CR+CO+ES+PL+VIX Addon running on port ${PORT}`);
    console.log(`Manifest: ${BASE_URL}/manifest.json`);
  });
}).catch(e => {
  console.error('Failed to init:', e.message);
  app.listen(PORT, () => {
    console.log(`CR+CO+ES+PL+VIX Addon running (no channels loaded yet) on port ${PORT}`);
  });
});
